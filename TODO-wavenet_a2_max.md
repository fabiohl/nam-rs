...
Este documento substitui integralmente os antigos `TODO-parity.md`, `TODO-findings.md` e
`TODO-sprints.md`. Todo o histórico de achados já **resolvidos** (F1 — bias de identidade do
FiLM; F2 — bugs B1/B2/B3 de aplicação de FiLM em `input_mixin_post_film`/`layer1x1_post_film`; F3
— `input_mixin_pre_film`/`head1x1_post_film`) foi auditado, confirmado como corrigido e commitado
(`git log`: `2108b3f`, `cc892ae`, `8a2de15`, `9312124`, `14612c1`, `ca8c22e`, `155e3db`, `d0d796e`,
`3d921af`) e por isso **não é repetido aqui** — consulte o histórico do git para os detalhes
dessas correções já encerradas. Este documento foca **exclusivamente** no único item ainda aberto:
o modelo flagship `wavenet_a2_max.nam`, hoje bloqueado em produção pelo guard
`is_disabled_broken_a2_flagship`.

**Estado do repositório no momento desta auditoria:** working tree limpo, `git status` sem
alterações staged/unstaged. Nenhum resíduo dos alertas de auditoria anteriores (contornos
temporários de guard, edições exploratórias em `build.rs`/`process_cascade.rs`) permanece no
código — foram corretamente descartados antes desta sessão.
