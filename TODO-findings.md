...
- **Epic 1.2: Refatoração Resiliente de Alocadores (Refere-se ao Finding 1)**
  - **Task 1.2.1 (`huge_alloc.rs`):** Inspecionar e alterar profundamente a função dependente de `Layout::from_size_align()`. Remover de forma terminante a chamada encadeada de `.unwrap()` e `.expect()`. Propagar o `Result::Err(alloc::LayoutError)` devidamente encapsulado num `Error::OutOfMemory` ou falhar de maneira branda, instanciando vetores zerados mas alertando às camadas superiores que não há memória contígua passível de alocação no host.
  - **Task 1.2.2 (`aligned.rs`):** Executar exata substituição referida na Task 1.2.1. Certificar que construtores atípicos como o de `AlignedVec` adotem os padrões idiomáticos do Rust, passando de instanciadores infalíveis para assinaturas que retornam obrigatoriamente `Result<AlignedVec, Error>`, e propagar/cascatear esta exigência a todos os módulos filhos que os instanciam.
