...
### Medição T3.9 (2026-07-10): Impacto isolado de B1/B2 sobre `wavenet_a2_max.nam`

**Metodologia:** Reexecução do teste `test_golden_vectors_wavenet_a2_max` (removendo
`#[ignore]` temporariamente) com o motor já corrigido por T3.1-T3.3 (B1/B2/B3),
contra o golden C++ `golden_wavenet_a2_max.bin` existente.

**Resultados:**

| Métrica | Antes de T3.1-T3.3 (Achado 2 original) | Depois de T3.1-T3.3 (2026-07-10) | Variação         |
| ------- | -------------------------------------- | -------------------------------- | ---------------- |
| MSE     | 2.46e3                                 | 7.30e3                           | **~3× pior**     |
| SNR     | −15.6 dB                               | −20.3 dB                         | **−4.7 dB pior** |
| ESR     | 3.61e1 (36.1)                          | 1.07e2 (107)                     | **~3× pior**     |
| MR-STFT | 3.41 (não medido nesta execução)       | 2.73                             | —                |

**Interpretação:**

A correção dos bugs B1 (`input_mixin_post_film` modulava `conv+mixin` em vez de só
`mixin`), B2 (`layer1x1_post_film` aplicado incondicionalmente em vez de só no modo
`BLENDED`) e B3 (`layer1x1_post_film` modulava `input+l1x1` em vez de só `l1x1`)
**não reduziu a divergência** do `wavenet_a2_max.nam` contra o golden C++. Pelo
contrário, a divergência **aumentou** em aproximadamente 3× em todas as métricas.

Este resultado negativo é informativo e consistente com o diagnóstico já estabelecido:

- O bug dominante em `wavenet_a2_max.nam` é o `condition_dsp` (Achado 2, §4.4 do
  `cpp_parity_map.md`), que afeta o processamento interno do submodelo DSP
  (`condition_size=8`, `head_size=8`, `head1x1.groups=2`).
- O fato de a correção de B1/B2/B3 ter **piorado** o ESR (em vez de melhorar)
  indica que os bugs de FiLM estavam **acidentalmente compensando** parte do erro
  introduzido pelo `condition_dsp`. Com o FiLM agora correto mas o `condition_dsp`
  ainda quebrado, não há mais cancelamento parcial de erro — a divergência líquida
  aumenta.
- **Conclusão:** B1/B2/B3 **não são** a causa raiz da divergência do
  `wavenet_a2_max.nam`. O `condition_dsp` (§4.4) permanece como o único bloqueador
  real. As correções de B1/B2/B3 são necessárias e corretas para qualquer modelo
  FiLM que não use `condition_dsp`, mas são insuficientes — e contraproducentes
  isoladamente — para este modelo flagship específico.

**Recomendação:** Prosseguir com Sprint 4 (`condition_dsp`) sem alterar a estratégia.
O guard `is_disabled_broken_a2_flagship` deve permanecer até que o `condition_dsp`
seja integralmente corrigido e verificado contra o golden.
