...
## Sprint 1: Núcleo FFT Nativo (`src/math/dsp/fft.rs`) [DONE]
...
## Sprint 2: Integração RT — Cabsim ConvEngine (`src/dsp/cabsim/conv.rs`) [DONE]
...
- [x] **Tarefa 2.2 (Refatoração SoA Total)**: O buffer `fft_buf` (que era um array de blocos `Complex<f32>` alocados para acomodar os requisitos do `rustfft`) deve ser transformado em dois arrays escalares independentes: `fft_buf_re: AlignedVec<f32>` e `fft_buf_im: AlignedVec<f32>`, preservando o alinhamento de 64 bytes.
