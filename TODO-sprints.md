...
> **Referência arquitetural:** [plan-clap.md](plan-clap.md) (estudo original — não editar)
> **Plano de implantação:** Vide artifact `implementation_plan.md` na sessão de planejamento.
> **DAW primária:** Cockos REAPER (Linux). Bitwig Studio e Studio One virão após todas as sprints.
...
- [ ] **T0.1.1** — Criar `src/common/mod.rs` e mover módulos compartilhados
...
- [ ] **T0.1.2** — Criar `src/standalone/mod.rs` e mover módulos PipeWire
...
- [ ] **T0.1.3** — Criar `src/clap/mod.rs` (stub)
...
- [ ] **T0.1.4** — Refatorar `src/lib.rs` como hub mínimo de re-exports
...
- [ ] **T0.1.5** — Refatorar `src/main.rs` para entry-point mínimo
...
- [ ] **T0.1.6** — Atualizar todos os imports internos do crate
...
- [ ] **T0.1.7** — Atualizar imports em testes e benchmarks
...
- [ ] **T0.1.8** — Atualizar `utils/run-standalone.sh`
...
- [ ] **T0.1.9** — Validação final do Épico 0.1
...
- [ ] **T0.2.2** — Atualizar `docs/architecture.md`
...
- [ ] **T0.2.3** — Atualizar `docs/clap_integration.md`
...
- [ ] **T0.2.4** — Atualizar `docs/dependencies.md`
...
- [ ] **T0.2.5** — Validação final do Épico 0.2
...
- [ ] **T1.1.1** — Adicionar dependências CLAP via `cargo add`
...
- [ ] **T1.1.2** — Configurar `[lib]` para cdylib
...
- [ ] **T1.2.1** — Implementar `src/clap/descriptor.rs`
...
- [ ] **T1.2.2** — Implementar `src/clap/plugin.rs` (Skeleton)
...
- [ ] **T1.2.3** — Implementar `src/clap/processor.rs` (Bypass puro)
...
- [ ] **T1.2.4** — Exportar entry point CLAP no `src/lib.rs`
...
- [ ] **T1.2.5** — Criar script `utils/build-clap.sh`
...
- [ ] **T1.3.1** — Validar detecção no REAPER
...
- [ ] **T1.3.2** — Validar bypass no REAPER
...
- [ ] **T2.1.1** — Adaptar buffers CLAP → Pipeline DSP
...
- [ ] **T2.1.2** — Carregar modelo no `activate()`
...
- [ ] **T2.1.3** — Validar inferência neural no REAPER
...
- [ ] **T2.2.1** — Implementar `src/clap/params_clap.rs`
...
- [ ] **T2.2.2** — Sincronização de params via eventos CLAP (sample-accurate)
...
- [ ] **T2.2.3** — Implementar State (Save/Load)
...
- [ ] **T2.2.4** — Validação final Sprint 2
...
- [ ] **T3.1.1** — Implementar Audio Ports Extension
...
- [ ] **T3.1.2** — Implementar `clap.latency` Extension
...
- [ ] **T3.2.3** — Teste com block sizes variáveis (1–4096 samples)
  - Processar com diferentes buffer sizes configurados no REAPER
  - **Aceite:** Funcional em todos os tamanhos de bloco testados

- [ ] **T3.2.4** — Testes de integração automatizados (`tests/clap_integration_test.rs`)
...
- [ ] **T3.2.5** — Validação final Sprint 3
  - `utils/lints.sh` + `cargo test` — zero falhas
...
- [ ] **T4.1.1** — Adicionar dependências GUI via `cargo add`
...
- [ ] **T4.1.2** — Implementar `src/clap/gui.rs` — Extension CLAP GUI
...
- [ ] **T4.2.1** — Implementar painel de controle
...
- [ ] **T4.2.2** — Implementar visualizadores
...
- [ ] **T4.2.3** — Sincronização bidirecional GUI ↔ Host
...
- [ ] **T4.2.4** — Validação final Sprint 4
...
- [ ] **T5.1.1** — Implementar `clap.thread-pool` Extension
...
- [ ] **T5.1.2** — Stress Test multi-instância no REAPER
...
- [ ] **T5.1.3** — Otimização de memória por instância
...
- [ ] **T5.1.4** — Validação final Sprint 5
  - `utils/lints.sh` + `cargo test` — zero falhas
...
**Objetivo:** Preparar para release alpha pública v1.5.0-alpha.

---

### Épico 6.1 — Validação em DAWs

- [ ] **T6.1.1** — Validação final no REAPER
...
- [ ] **T6.1.4** — Validação no Presonus Studio One (Linux)
...
- [ ] **T6.2.1** — Documentação final do plugin
