...
### 🟡 E1.1 — `LoadedModels` Tuple → Struct Nomeada
...
### 🟡 E1.2 — `NamMetadata` Default via `impl Default`
...
### 🟢 E1.3 — Constantes Mágicas de Calibração
...
### 🟡 E2.1 — `GcOverflowBuffer` — Validação de Tamanho
...
### 🟢 E2.2 — Docs de `RtStatusFlags` — Diagrama de Bits
...
### 🟡 E3.1 — `run_inference` — Stack Array no Hot-Path com Resampler
...
### 🟡 E3.2 — `handle_silence_bypass` — Ordering Inconsistente
...
### 🟡 E3.3 — `pipeline.rs` — Sinalização de Silêncio Invertida
...
### 🟡 E3.4 — `DspBridge` — Double-Buffer sem Backpressure
...
### 🟡 E3.5 — `gate.rs` — Parâmetro `_params` Ignorado em `apply_gain_rt`
...
### 🟢 E3.6 — `telemetry.rs` — Histograma Sem Proteção Contra Overflow
...
### 🟢 E3.7 — `vring.rs` — `VirtualRingBuffer` Não Implementa `Debug`
...
### 🟡 E4.1 — `DelayLine` Usa `Vec` — Potencial para `AlignedVec`
...
### 🟡 E4.2 — `sinc_kernel.rs` — `AlignedCoeffs._len` Nunca Usado
...
### 🟢 E4.3 — `gcd()` — Função Pública Sem Consumidores Internos Visíveis
...
### 🟡 E5.1 — Duplicação Massiva entre `process_single_frame` e `process_single_frame_bf16`
...
### 🟡 E5.2 — `DenseLayer::process_acc_single_frame` e `process_fused` São Idênticos
...
### 🟡 E5.3 — `DenseLayer::process_acc_block` e `process_fused_block` São Idênticos

- **Arquivo**: `wavenet.rs:749-789`
- **Achado**: Mesmo caso do E5.2 mas para a variante batch.
- **Proposta**: Idem E5.2.

### 🟡 E5.4 — `wavenet_common.rs` — `assert!` no Hot-Path
...
### 🟡 E5.5 — `wavenet_dyn.rs` — Duplicação entre `process` e `prewarm`
...
### 🟢 E5.6 — `loader/dispatcher/wavenet.rs` — Duplicação de Lógica de Quantização
...
### 🟡 E6.1 — `lstm.rs` — Macro Gera Funções com Assinatura Inconsistente
...
### 🟡 E6.2 — `lstm.rs` — Fallback Escalar Usa `half::f16` Sem SIMD
...
### 🟡 E6.3 — `lstm_dyn.rs` — Portas LSTM Separadas em Loops Distintos
...
### 🟢 E6.4 — `models/mod.rs` — Prewarm Duplicado para LSTM 1 e 2 Camadas
...
### 🟡 E7.1 — `dispatch_simd!` Macro — Fallback Usa AVX2
...
### 🟡 E7.2 — Trait `SimdMath` — Superfície API Grande
...
### 🟢 E7.3 — `fastmath.rs` — Verificar Paridade Numérica dos Polinômios Minimax
...
### 🟡 E8.1 — `loader/dispatcher/wavenet.rs` — `validate_layer_activations` Limitada
...
### 🟢 E8.2 — Erro de I/O Sem Contexto de Errno
...
### 🟡 E9.1 — Cobertura de Testes para `pipeline.rs`
...
### 🟡 E9.2 — Golden Vectors para Paridade C++
...
### 🟢 E10.1 — `WavenetA2Placeholder` — Placeholder Silencioso
...
### 🟢 E10.2 — Stubs `film.rs` e `gating.rs` — Sem Implementação
...
### 🟢 E10.3 — `wavenet_params.rs` — Estruturas Completas sem Consumidores
