...
* **Arquivos Afetados**:
  ...
  * [golden_vectors.rs](file:///home/fabio/nam-rs/tests/models/golden_vectors.rs)
  * [cpp_parity.rs](file:///home/fabio/nam-rs/tests/parity/cpp_parity.rs)
  * [cabsim_cpp_parity.rs](file:///home/fabio/nam-rs/tests/parity/cabsim_cpp_parity.rs)
  * [linear_fft_test.rs](file:///home/fabio/nam-rs/tests/models/linear_fft_test.rs)
* **Descrição**:
...
* **Critério de Aceitação (DoD)**:
  ...
  **Conclusão (2026-07-12)**: `mse_limit` migrado de `f64` para `Option<f64>` em toda a cadeia: `get_calibrated_threshold` → `topology_thresholds`/`live_parity_thresholds` → `report_dsp_fidelity`. Os 3 modelos que usavam `1e30` (wavenet_a2_full, wavenet_a2_lite, a2_example) agora retornam `None` (semântica `MseGate::NotApplicable`). A renderização mostra `gate: N/A — ESR primary` em vez do placeholder mágico. As asserções de MSE são condicionais (`if let Some(limit)`), e as relaxações v2 em golden_vectors.rs e cpp_parity.rs usam `if let Some(ref mut m)` para aplicar ajustes apenas quando o gate MSE está ativo. O meta-teste anti-placebo em threshold_calibration.rs verifica `mse_limit.is_none()` com requisito de ESR/SNR rígidos (SNR ≥ 40 dB, ESR < 0.1). 36/37 golden_vectors passam (falha única: wavenet_condition_lstm, pré-existente). 8/8 threshold_calibration passam. Lints limpos.
