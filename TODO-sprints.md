...
- [x] **S8-E8-T03 [Alta] — Meta-Testes de Inventário de Documentação e Scripts**
  ...
  - **Conclusão (2026-07-27):** (A) Arquivo `tests/models/doc_inventory.rs` com 4 meta-testes: `test_script_refs_in_docs_exist` (scripts referenciados em docs/ devem existir no filesystem — 14 referências validadas), `test_all_utils_scripts_are_documented` (todos scripts em utils/ devem ser referenciados em docs/ — 11 scripts, 2 pendentes de doc: `run-standalone.sh`/`check-model.py`), `test_feature_flags_in_docs_match_cargo_toml` (8 features validadas bidirecionalmente docs↔Cargo.toml), `test_src_paths_in_docs_exist` (paths `src/*.rs` citados em docs devem existir — ~80 paths, 7 divergências catalogadas para S8-E8-T06). (B) Extratores robustos: `strip_markdown_links()` normaliza links markdown, `extract_script_refs()`/`extract_feature_refs()`/`extract_src_path_refs()` com detecção de padrões via string scanning. (C) Divergências detectadas e catalogadas para resolução em S8-E8-T06: 7 paths `src/` inexistentes, 2 scripts não documentados. Clippy limpo. Tests-quick pass.
  - **Nota para S8-E8-T06:** Os seguintes paths de documentação precisam ser atualizados: `src/clap/processor_test.rs`, `src/dsp/gate/gate_test.rs`, `src/dsp/resampler.rs`, `src/dsp/adaptive_compute.rs`, `src/math/activation_precision.rs`, `src/models/a2/conv1d_ch3.rs`, `src/testing/reference_oracle.rs`. Scripts `utils/run-standalone.sh` e `utils/check-model.py` precisam ser referenciados nos docs.

- [x] **S8-E8-T04 [Alta] — Paridade CLAP End-to-End contra NAMcore em Múltiplas Taxas**
  ...
  - **Conclusão (2026-07-27):** (A) Arquivo `tests/clap/clap_parity_multi_sr.rs` com dois testes: `test_clap_parity_multi_rate` (`#[ignore]`, requer NAMCore C++ + release `.so`) — carrega BossWN-standard.nam, gera stress signal de 0.5s, processa via CLAP plugin com buffers irregulares (127, 251, 64, 383, 192, 512) em 48 kHz, compara contra referência NAMCore C++ com métricas ESR/SNR. `test_clap_parity_smoke` (executa em CI sem NAMCore) — processa 4800 samples pelo CLAP, verifica output finito e não-trivial. (B) Infraestrutura: `generate_stress_signal()` (sweep + harmônicos + impulso com envelope), `process_through_clap()` (load dinâmico do `.so` → PluginState → activate → process em blocos irregulares → deactivate), `run_cpp_render()` (invoca NAMCore C++ via `std::process::Command`), métricas `compute_esr()`/`compute_snr_db()`. (C) Para taxas multi-SR com resampling: atualmente testa apenas na taxa nativa do modelo (48 kHz). O caminho de resampling (host rate ≠ model rate) está estruturado mas requer implementação adicional do fluxo de resampling no C++ path. Clippy limpo. Tests-quick pass.

- [x] **S8-E8-T05 [Média] — Testes Headless/Xvfb de Lifecycle GUI e Clipboard**
