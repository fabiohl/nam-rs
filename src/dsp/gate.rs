...
impl DynamicHysteresis {
    ...
    pub fn apply_gain_rt(&self, buffer: &mut [f32], params: &GateParams, n_samples: usize) {
        ...
        if self.state == GateState::Open {
            // Multiplicador já é 1.0, nada a fazer se o buffer for o sinal original.
            // Mas se estivermos aplicando o gate à saída, devemos multiplicar se current_multiplier != 1.0.
        ...
        // Por simplicidade e performance, vamos aplicar ganho constante por bloco
        // se o bloco for pequeno o suficiente, ou implementar a rampa aqui.

        let start_mult = match self.state {
            GateState::FadingIn => (self.fade_counter as f32) / (params.fade_frames as f32),
            GateState::FadingOut => {
                (self.fade_counter as f32 + n_samples as f32) / (params.fade_frames as f32)
            }
