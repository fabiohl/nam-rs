...
use baseview::{Event, EventStatus, MouseButton, MouseEvent, ScrollDelta, Window, WindowEvent, WindowHandler};
use std::sync::Arc;
...
impl NamPluginWindow {
    ...
    pub fn new(window: &mut Window) -> Self {
        ...
        let gl = unsafe {
            glow::Context::from_loader_function(|s| gl_ctx.get_proc_address(s))
        };
        ...
        let full_output = self.egui_ctx.run(raw_input, |ctx| {
            egui::CentralPanel::default().show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.heading("NAM-rs");
                    ui.add_space(10.0);
                    ui.label("Interface gráfica inicializada com baseview + egui + glow");
                    ui.add_space(30.0);
                    ...
                    ui.horizontal(|ui| {
                        ui.columns(4, |cols| {
                            cols[0].vertical_centered(|ui| {
                                ui.label("Input Gain");
                                ui.add(egui::Slider::new(&mut 0.0, -20.0..=20.0).show_value(true));
                            });
                            cols[1].vertical_centered(|ui| {
                                ui.label("Gate Threshold");
                                ui.add(egui::Slider::new(&mut -60.0, -100.0..=0.0).show_value(true));
                            });
    ...
    fn on_event(&mut self, _window: &mut Window, event: Event) -> EventStatus {
        match event {
            ...
            Event::Mouse(mouse_event) => {
                match mouse_event {
                    MouseEvent::CursorMoved { position, modifiers: _ } => {
                        ...
                        self.raw_input.events.push(egui::Event::PointerMoved(egui_pos));
                    }
                    MouseEvent::ButtonPressed { button, modifiers } => {
                        if let Some(egui_button) = map_mouse_button(button) {
                            let last_pos = self.raw_input.events.iter().rev().find_map(|e| {
                    ...
                    MouseEvent::WheelScrolled { delta, modifiers } => {
                        let (unit, delta_vec) = match delta {
                            ScrollDelta::Lines { x, y } => (egui::MouseWheelUnit::Line, egui::vec2(x * 10.0, y * 10.0)),
                            ScrollDelta::Pixels { x, y } => (egui::MouseWheelUnit::Point, egui::vec2(x, y)),
                        };
            ...
            Event::Keyboard(key_event) => {
                ...
                if pressed {
                    if let keyboard_types::Key::Character(s) = &key_event.key {
                        if !key_event.modifiers.contains(keyboard_types::Modifiers::CONTROL) &&
                           !key_event.modifiers.contains(keyboard_types::Modifiers::META) {
...
fn map_modifiers(mods: keyboard_types::Modifiers) -> egui::Modifiers {
    egui::Modifiers {
        ...
        command: mods.contains(keyboard_types::Modifiers::META) || mods.contains(keyboard_types::Modifiers::CONTROL),
    }
}
