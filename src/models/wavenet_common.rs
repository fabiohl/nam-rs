...
pub struct WavenetDynProcessContext<'a> {
    ...
    /// Acumulador para a cabeça de saída (skip connection).
    pub head_input: &'a mut [f32],
    /// Buffer de saída da camada (residual path).
...
impl WaveNetLayerDyn {
    ...
    pub unsafe fn process_block_internal<M: SimdMath>(&self, ctx: WavenetDynProcessContext<'_>) {
