...
#[test]
fn test_scalar_ref_fused_add_gemv() {
    ...
    unsafe {
        ScalarRefMath::fused_add_gemv(&in_frame, &w_u16, &bias, &mut out_frame, true);
    }
...
#[test]
fn test_scalar_ref_gemv_overwrite_4gate() {
    let in_frame = vec![1.0, 2.0];
    let hidden_size = 2;
    // 4 gates x hidden_size = 8 outputs total
    // Re-organize weights for the layout: (in_len * out_total)
    // Actually ScalarRefMath::gemv_overwrite_4gate uses gemv_overwrite_fallback internally.
    ...
    unsafe {
        ScalarRefMath::gemv_overwrite_4gate(
            &in_frame,
            &w_u16,
            &bias,
            &mut out_gates,
...
fn test_scalar_ref_lstm_gates() {
    ...
    unsafe {
        ScalarRefMath::fused_lstm_gates_dyn(
            &mut gates,
            &mut cell_state,
            &mut hidden_state,
            hidden_size,
...
#[test]
fn test_scalar_ref_activations() {
    let mut data = [-1.0, 0.0, 1.0];
    let mut data_copy = data;

    unsafe {
        ScalarRefMath::tanh_slice(&mut data);
        ScalarRefMath::sigmoid_slice(&mut data_copy);
    }
...
#[test]
fn test_scalar_ref_apply_gain_and_detect_clipping_stereo() {
    ...
    let clipped = unsafe {
        ScalarRefMath::apply_gain_and_detect_clipping_stereo(&mut left, &mut right, gain)
    };
    ...
    let gain2 = 0.5;
    let clipped2 = unsafe {
        ScalarRefMath::apply_gain_and_detect_clipping_stereo(&mut left2, &mut right2, gain2)
    };
    assert!(!clipped2);
...
#[test]
fn test_scalar_ref_compute_energy_stereo() {
    ...
    let result = unsafe { ScalarRefMath::compute_energy_stereo(&l, &r) };
    assert!((result - 7.5).abs() < 1e-6);

    let l2 = [0.1, 0.2];
    let r2 = [1.0, 2.0]; // energy_r2 = (1 + 4)/2 = 2.5
    let result2 = unsafe { ScalarRefMath::compute_energy_stereo(&l2, &r2) };
