...
fn test_leaky_relu_scalar() {
    ...
    assert!((leaky_relu(-3.0) - (-0.03)).abs() < 1e-7);
    assert_eq!(leaky_relu(0.0), 0.0);
    assert_eq!(leaky_relu(-0.0), 0.0);
}

#[test]
fn test_leaky_relu_slice_dispatch_smoke() {
    let mut data = vec![1.0, -2.0, 3.0, -4.0, 0.0, -0.0, 5.0, -1.0, 0.5, -0.5];
    let original = data.clone();
    leaky_relu_slice(&mut data);
    assert!((data[0] - original[0]).abs() < 1e-7);
    assert!((data[1] - (original[1] * 0.01)).abs() < 1e-7);
    assert!((data[2] - original[2]).abs() < 1e-7);
    assert!((data[3] - (original[3] * 0.01)).abs() < 1e-7);
    assert_eq!(data[4], 0.0);
    assert_eq!(data[5], 0.0);
    assert!((data[7] - (original[7] * 0.01)).abs() < 1e-7);
}

#[test]
fn test_leaky_relu_slice_parity_scalar_ref() {
    use crate::math::common::scalar_ref::utility::leaky_relu_slice_fallback;

    // Test with various lengths to cover both SIMD and remainder paths.
    for len in [0, 1, 3, 7, 8, 9, 15, 16, 17, 23, 32, 64, 100, 127, 256] {
        let mut data_simd: Vec<f32> = (0..len)
            .map(|i| (i as f32 - len as f32 * 0.5) * 0.137)
            .collect();
        let mut data_ref = data_simd.clone();

        leaky_relu_slice(&mut data_simd);
        unsafe {
            leaky_relu_slice_fallback(&mut data_ref);
        }

        for (i, (&a, &b)) in data_simd.iter().zip(data_ref.iter()).enumerate() {
            let error = (a - b).abs();
            assert!(
                error < 1e-6,
                "leaky_relu parity[{i}]: simd={a}, ref={b}, delta={error}, len={len}"
            );
        }
    }
}

// Proptest: validates SIMD LeakyReLU(0.01) slice against scalar reference.
// 10k random slices of varying lengths.
proptest! {
    #[test]
    fn test_leaky_relu_slice_proptest(
        values in prop::collection::vec(-5.0f32..5.0f32, 0..512)
    ) {
        use crate::math::common::scalar_ref::utility::leaky_relu_slice_fallback;

        let mut data_simd = values.clone();
        let mut data_ref = values;

        leaky_relu_slice(&mut data_simd);
        unsafe { leaky_relu_slice_fallback(&mut data_ref); }

        for (i, (&a, &b)) in data_simd.iter().zip(data_ref.iter()).enumerate() {
            let error = (a - b).abs();
            prop_assert!(
                error < 1e-6,
                "leaky_relu[{i}]: simd={a}, ref={b}, delta={error}"
            );
        }
    }
}
