...
fn make_data(len: usize) -> (Vec<[u16; 4]>, Vec<f32>) {
    let weights: Vec<[u16; 4]> = (0..len)
        .map(|i| {
            ...
            [bits, bits.wrapping_mul(3), bits.wrapping_add(0x1000), bits.wrapping_mul(7)]
        })
        .collect();
...
fn test_dot_4x_interleaved_avx512_vs_fallback() {
    ...
    let sizes = [0, 1, 2, 3, 4, 5, 7, 8, 9, 15, 16, 17, 31, 32, 33, 64, 127, 128, 255, 256, 512];
    for &len in &sizes {
...
fn test_dot_4x_interleaved_avx512_vs_avx2() {
    ...
    for &len in &sizes {
        ...
        let avx2_result = unsafe {
            dot_product_4x_interleaved_avx2(&weights, &state)
        };
...
fn test_dot_4x_interleaved_dual_frame_avx512_vs_avx2() {
    ...
    for &len in &sizes {
        ...
        let avx2_result = unsafe {
            dot_product_4x_interleaved_dual_frame_avx2(&weights, &state_f0, &state_f1)
        };
        let avx512_result = unsafe {
            dot_product_4x_interleaved_dual_frame_avx512(&weights, &state_f0, &state_f1)
        };
