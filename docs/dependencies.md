...
| **`rubato`**                   | `0.16.x`      | Resampler FIR Sinc para conversão bidirecional de sample rate (PW↔48 kHz). Usa Kaiser-BlackmanHarris2 com interpolação Hermite cúbica. A versão 0.16 oferece API `Vec<Vec<f32>>` compatível com buffers pré-alocados sem alocações no callback RT. | *rubato 2.0*: exige `audioadapter` + `audioadapter-buffers` (+2 crates obrigatórios) sem benefício RT.      |
