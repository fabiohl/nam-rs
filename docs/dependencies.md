...
| **`rustfft`**                    | `^6.4`         | High-performance FFT algorithm. Used offline (outside the RT thread) in: `NamResampler::new()` (minimal-phase transformation via Real Cepstrum), `dsp/cabsim` (cabinet simulation convolution), `dsp/sinc_kernel` (sinc table generation), and `testing/perceptual` (MR-STFT for cross-validation metrics). | *realfft*: Wrapper over rustfft; we prefer direct rustfft with `default-features = false`.    |
...
| **`cargo-show-asm`**     | `cargo install cargo-show-asm`            | Compiler-grade optimization assistant. Allows developers to inspect assembly, LLVM-IR, and MIR generated by the Rust compiler.                                         |
