...
**Known limitation:** LSTM fused 4-gate GEMV kernels bypass the `ActivationPrecision` dispatch and always use the Standard path. The fused kernel layout is architecturally incompatible with the HighFidelity exp-kernel structure. Full dispatch coverage extends to WaveNet (A1 + A2), ConvNet, and Linear models. **LSTM HighFidelity gate kernel coverage is deferred to Épico β (I6)** — the runtime mode switch is functional and zero-alloc for all model families, but LSTM models silently ignore the mode change.
