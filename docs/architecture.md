...
- **Multiversioning AVX-512 — LazyLock V-Table:** Despacho via `SimdMathConfig::get()` retorna `&'static SimdMathConfig` cacheado em `LazyLock`, eliminando a leitura atômica de `is_x86_feature_detected!` que ocorria a cada bloco DSP. A inicialização ocorre **uma única vez** no primeiro acesso, e subsequentes chamadas são simples carga de ponteiro (overhead zero). LSTM estático usa comparação de function pointer (`dot_product as usize == dot_product_avx512 as *const ()`) para dispatch AVX-512 vs AVX2. Ver `src/math/simd.rs`.
...
> **Decisão Arquitetural (Lição Aprendida com as Sprints de LSTM):** A tentativa de forçar paralelismo extremo (GEMV Fused Gate + Prefetch L1) no LSTM resultou em ganhos marginais (~1%) sob o teto de latência. O motor superscalar de CPUs modernas (*Out-of-Order Execution* e *Hardware Prefetcher*) já esgota a capacidade de vetorização inter-amostras no LSTM. Para latências sub-milisegundo, o projeto desloca o foco de otimização pesada para as topologias **WaveNet**. Diferente da LSTM (recursividade IIR onde `t` depende estritamente de `t-1`), a convolução dilatada da WaveNet (estrutura FIR) permite processamento em bloco (Batch Processing), destravando um teto matemático brutalmente mais alto usando matrizes e cache L1 de forma efetiva.
...
| `src/models/wavenet.rs`     | WaveNet CNN causal dilatada — SoA com const generics, Conv1d + DenseLayer + prewarm copy_buffer. Software prefetch proativo (`_mm_prefetch`) nos taps dilatados do kernel Conv1d para eliminar L1 cache misses previsíveis.                                                                                   |
...
| `src/dsp/gain.rs`           | Gain staging SIMD (AVX2 `_mm256_mul_ps`) baseado em metadados `input/output_level_dbu`. `detect_clipping_stereo_simd` detecta saturação estéreo via AVX2 (8 samples/iteração) substituindo loop escalar.                                                                                                      |
