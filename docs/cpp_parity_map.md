...
- **A2 FiLM / RF1 (PM-03).** ✅ **Resolved (S10.3)** — the f64 oracle now models FiLM completely
  ...
  vs f64 oracle yields ESR at the numerical floor: 9.52e-15 (−140.2 dB) for FilmLite, 1.15e-14
  (−139.4 dB) for FilmFull. This proves the Rust FiLM implementation is **mathematically correct**.
  The 18-36 dB SNR gap vs C++ is **inherent structural divergence**: the C++ generic WaveNet
  (Eigen-based, used as fallback when `a2_fast.cpp` rejects FiLM) applies conditioning through a
  structurally different path from the Rust native FiLM-A2 engine. This is analogous to the HF
  activation asymmetry (§4.5): closer to the mathematical ideal, further from C++ bit-equivalence.
  RF1 reclassified from 🔴 (suspected bug) to 🟡 (inherent, documented, capped).
...
- **`SlimmableWavenet` (PM-06).** Genuinely deferred — the practical use case (adaptive quality) is already covered by the implemented and tested `SlimmableContainer` (`src/models/container.rs`, `tests/container_slimmable.rs`), which achieves the same goal via independent sub-networks with crossfade orchestration rather than single-net channel slicing. The two constructs serve distinct design purposes:

  *`SlimmableContainer`* (🟢 implemented): hosts multiple complete `.nam` models — each a self-contained network with its own channel configuration — and seamlessly crossfades between them at runtime. This is a model-orchestration layer, not a weight-slicing mechanism.

  *`SlimmableWavenet`* (🟡 deferred): a single `.nam` file encodes an over-parameterized WaveNet using the widest channel count, from which narrower channel variants are extracted at load time by slicing weights, biases, and state buffers. This is a weight-serialization and slicing mechanism — a fundamentally different concept from multi-model orchestration.

  Any future implementation of `SlimmableWavenet` must satisfy the following explicit acceptance criteria:

  1. **Parser para múltiplas larguras de canal em um único arquivo `.nam`.** The `.nam` JSON format must be extended (or the existing over-parameterized convention adopted from NAMCore) so that weight tensors encode the widest channel configuration and the parser extracts per-channel-width projections at load time, producing structurally valid catalog SKUs (Standard=16, Lite=12, Feather=8, Nano=4) from the single file.
...
- **Test observability (PM-14).** 🟢 Resolved by PO Decree (S12.5, 2026-06-30). The 4 guard-script audit (`tests-long.sh`, `tests-quick.sh`, `tests-performance-regression.sh`, `build-release.sh`) was reviewed in full (see `TODO-findings.md` PM-14), confirming: (a) comprehensive golden pre-flight (v1+v2 per SR group), no-fail-fast aggregation with per-phase summary, golden freshness manifesto, heap audits, RT gates, and CLAP validation in `tests-long.sh`; (b) strict clippy + SHA256 integrity of the CLAP binary between phases in `tests-quick.sh`; (c) core pinning (`taskset`) + criterion baseline in `tests-performance-regression.sh`; (d) graceful PGO+BOLT degradation in `build-release.sh`. **Decreto do PO (2026-06-30):** O PO decidiu formalmente que a infraestrutura de observabilidade e os scripts-guardião existentes são suficientemente robustos e que a referência do GitHub mais recente é a fonte canônica ("e ponto final"). Nenhuma asserção rígida de pin de commit é imposta (PM-09), nenhuma captura adicional de linha-sumário do harness `lib` é exigida, e nenhuma alteração na bateria de observabilidade ao vivo é necessária — o fluxo de trabalho `tests-long.sh` executado pelo desenvolvedor é preservado como está. O Épico J é classificado como `Won't Do` por decisão executiva do PO.
