...
- **FiLM** (`wavenet_a2_film_{full,lite}.nam`, synthetic, generated by `generate_a2_fixtures.py`):
  Near-bit-exact parities are now achieved following the correction of the fixture weight generator.
  Previously, a calibrated gap of SNR = 36.0 dB (Full) / 18.1 dB (Lite) was reported. This was shown to
  be a fixture-generation issue: the Python generator did not apply the standard `+1.0` bias to the
  FiLM `scale` channels, leading to a scale multiplier near zero. This squashed the output signal energy
  while keeping the absolute rounding error (~1e-7) constant, inflating the relative ESR.
  With the corrected generator applying `scale ≈ 1.0` (identity-biased), the interop gap collapsed to the
  numerical floor:
  - **FiLM Lite (CH=3):** SNR = **138.3 dB**, ESR = **1.48e-14** (standard threshold calibrated at 120.0 dB / 1.0e-11).
  - **FiLM Full (CH=8):** SNR = **138.8 dB**, ESR = **1.31e-14** (standard threshold calibrated at 120.0 dB / 1.0e-11).
  As a mitigation and to stress-test the engines under severe numerical chaos, the original zero-biased fixture
  was preserved under `wavenet_a2_film_chaos_stress.nam`. This serves as a numerical stress test verifying
  that both engines behave consistently even under degenerated signal conditions (gated by a dedicated
  relaxed threshold: SNR = 12.0 dB, ESR = 3.5e-2).
