...
`sigmoid`/`tanhf`: NAM-rs's `ActivationPrecision::Standard` uses a Padé [5,4] rational tanh
...
weight quantization). `HighFidelity` activation mode (polynomial exp-based tanh/sigmoid,
error ≈ 2×1e-7) trades interop parity for closer-to-ideal math — see `audio_fidelity_map.md` §2.
