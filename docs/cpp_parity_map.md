...
1. **Interop parity** — does NAM-rs match NAMcore bit-for-bit (within float tolerance)? Verified
   ...
   and live cross-validation (`tests/cpp_parity.rs`).

   > **Correction (2026-07-05):** NAM-rs's f16c/bf16 backbone weight  quantization has been **removed**.
   > NAMcore never quantized (verified against the vendored source, §2.5 below); this document previously
   > claimed "both engines use f16c/bf16-quantized weights internally" — that was wrong and has
   > been retracted. NAM-rs **now operates in native f32**, converging with NAMcore rather than
   > carrying an asymmetric precision floor. §2.5 details the measured results: BossLSTM-2×8
   > converged to bit-exact interop parity (ESR = 0.00e0); BossLSTM-1×16 was unchanged (ESR =
   > 1.04e-2 before and after), proving that f16c quantization was **not** the dominant source
   > of that model's interop drift.
...
- **FiLM** (`wavenet_a2_film_{full,lite}.nam`, synthetic, generated by `generate_a2_fixtures.py`):
  Near-bit-exact parity was achieved in two phases. **Phase 1 (Sprint S1/S2):** The fixture weight
  generator was corrected to apply the standard `+1.0` bias to the FiLM `scale` channels, collapsing
  the interop gap from SNR 18.1–36.0 dB to ~138 dB with 2 active slots (`conv_post_film` +
  `activation_post_film`). **Phase 2 (Sprint S3, T3.1-T3.8):** Three production bugs (B1/B2/B3) in
  `input_mixin_post_film` and `layer1x1_post_film` were corrected, the remaining 2 slots were
  restored to the fixtures, and goldens were regenerated with all 4 slots active. Final measurements
  (2026-07-10, commit `ca8c22e`, T3.7):
  - **FiLM Full (CH=8, 4 slots):** SNR = **139.4 dB**, ESR = **1.15e-14**, MR-STFT = 3.52e-5
    (threshold calibrated at 120.0 dB / 1.0e-11).
  - **FiLM Lite (CH=3, 4 slots):** SNR = **124.2 dB**, ESR = **3.83e-13**, MR-STFT = 2.43e-5
    (threshold calibrated at 120.0 dB / 1.0e-11).
  - **FiLM Chaos Stress (CH=3, 4 slots, zero-biased):** SNR = **139.0 dB**, ESR = **1.25e-14**
    (threshold calibrated at 12.0 dB / 3.5e-2).
    The Lite model's drop from 138.3 dB (2-slot) to 124.2 dB (4-slot) is expected — the two
    additional slots (`input_mixin_post_film` + `layer1x1_post_film`) are now active and correct,
    applying additional FiLM modulation that exercises f32 round-off error. Margin remains adequate
    at 4.2 dB above the 120 dB gate.
