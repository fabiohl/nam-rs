...
1. **Interop parity** — does NAM-rs match NAMcore bit-for-bit (within float tolerance)? Verified
   ...
   and live cross-validation (`tests/cpp_parity.rs`). Both engines use f16c/bf16-quantized
   weights internally, so this proves *agreement*, not absolute mathematical fidelity.
...
otherwise; C++ keeps everything in native `float`. This is the **dominant** source of LSTM
interop drift — confirmed by the oracle decomposition in [§2.7](#27-measured-interop-drift):
production-vs-oracle ESR (3.57e-3) is roughly two orders of magnitude worse than WaveNet A1's
(6.13e-14), where no such quantization exists. The head projection is deliberately kept in
native f32 (Kahan-compensated) specifically to avoid compounding this error at the output stage.
...
- **LSTM backbone weight quantization** (bf16/f16 vs. C++'s native `float`) — the dominant,
  understood source of LSTM interop drift, with sample-rate-scaled ESR gates that hold with
  margin at every supported rate (§2.5, §2.7).
