...
| **`meta_coherence`**                | Integration | *None*                      | No                        | No                       | No                              | **Yes** (Pre-flight, blocking)       | Meta-test asserting golden-catalog ↔ ignored-test model coherence. Runs before Phase 1 so catalog drift fails fast, not after a wasted nightly window. |
...
|:--------------------------------------------- |:---------------------------------------------------------------------------------------------------------------------- |:----------------------------------------------------------------------------------------------------------------------------------------------------- |:--------------------------------------------------------- |:--------------------- |
| **`tests/meta_coherence.rs`**                 | `test_ignored_models_are_in_catalog`                                                                                   | Not itself slow — parses `golden_gen_build.sh` + test sources; grouped with the audit suite so catalog drift is caught before the nightly run starts. | Long Suite (Pre-flight, blocking)                         | Pre-release / Nightly |
...
| **`src/math/activations/tanh/`**              | `test_tanh_poly_nr*_vs_div_*`, `test_sigmoid_poly_*_sweep`, `test_pade_nr*_*`, `test_pade_nr1_dual_vs_production_avx2` | Relative consistency only (approx vs approx, no ground truth). f64 Oracle provides absolute correctness (T-CR2).                                      | Long Suite (Phase 3)                                      | Nightly               |
