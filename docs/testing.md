...
| **`adaptive_fsm_proptest`**         | Integration | *None*                      | **Yes** *(ignored)*       | No                       | No                              | **Yes** (Phase 2)                    | FSM state transitions under varying load and jitter scenarios                                                                        |
...
| **`diagnostic_bundle` (Heap)**      | Integration | `heap-audit`                | No                        | No                       | No                              | **Yes** (Phase 3)                    | Zero-alloc verification of diagnostic and telemetry operations                                                                       |
| **`gate_fsm_proptest`**             | Integration | *None*                      | No                        | No                       | No                              | **Yes** (Phase 2)                    | Property-based tests verifying the Gate finite state machine under load                                                              |
...
| **`pw_integration_test`**           | Integration | `standalone`                | No                        | No                       | No                              | **Yes** (Phase 6)                    | Integration testing of standalone runner connected to PipeWire daemon                                                                |
...
3. **Heap-Audit (release)**: Zero-alloc verification under the `heap-audit` global allocator — resampler, cabsim, A2, and the `diagnostic_bundle` heap variant. Plus full C++ parity and golden validation: `tests/cpp_parity.rs` (full matrix), `tests/cabsim_cpp_parity.rs`, `tests/golden_vectors.rs` (v2 multi-SR), `tests/linear_fft_test.rs` C++ goldens.
4. **Release-Mode CLAP Auditing & Concurrency**: Build the release `.so`, SONAME/symbol audit, `clap-validator` strict, `clap_lifecycle_test`, `clap_state_migration`, `clap_multi_instance` (ignored stress), `processor_gc_stress` (1000 swaps), `concurrency_stress`, and `cargo test --lib` mono-mode.
5. **Criterion Performance Benchmarks**: Measurement of DSP block runtime budgets and throughput limits (`benches/`).
6. **RT Characterization & PipeWire**: `rt_deadline` (release, deadline assertion) + `rt_jitter` (release, `--ignored` — all jitter tests) + `pw_integration_test` (live PipeWire daemon).
...
| **`tests/cpp_parity.rs`**                     | `live_cross_validation_*_lite`                                                                                         | **Skipped (Known Divergent)**: WaveNet Lite (CH=12) exhibits numerical drift against C++.                                              | Skipped                                                   | Deferred              |
...
| **`tests/golden_vectors.rs`**                 | `test_golden_vectors_wavenet_lite`, `test_golden_vectors_v2_wavenet_lite`                                              | **Ignored (Known Divergent)**: A1 WaveNet Lite (CH=12) exhibits drift (SNR = 0.9 dB) against C++.                                      | None                                                      | Deferred              |
