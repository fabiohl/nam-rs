...
- **Audio Thread (Real-time)**:
  ...
  - Utilizará `src/dsp/pipeline.rs` (especificamente `capture_dsp_pipeline`) para o processamento.
...
|:------------------ |:------------- |:------- |:------------------------------------------------ |
| **Input Gain**     | `input_gain`  | dB      | Ganho aplicado antes da inferência neural.       |
| **Output Gain**    | `output_gain` | dB      | Ganho aplicado após a inferência neural.         |
| **Gate Threshold** | `gate_thresh` | dB      | Limiar de abertura do Noise Gate.                |
...
**Recomendação (conforme `v1.4_research_and_planning.md`)**: `nih-plug` é a opção mais pragática para um plugin CLAP de alta qualidade — battle-tested e multi-formato. `clack` é preferível quando se deseja controle granular sem overhead de VST3. A decisão final pode ser diferida: o staging atual foca em tornar o NAM-rs *plugin-ready* via trait `AudioHost` sem depender de nenhum crate CLAP.
