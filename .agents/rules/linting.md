...
7. **Logging Audit**: Verify that all new or modified off-RT components (loaders, builders, plugin lifecycle, DSP config) include informative `log::*` calls, and strictly ensure zero `log::*` calls exist in hot-path audio processing routines.
